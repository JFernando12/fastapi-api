...
class ProjectService:
    ...
    def create_project(self, db: Session, project_data: ProjectCreate) -> dict:
    ...
    def update_project(self, db: Session, project_id: str, project_data: ProjectCreate) -> dict | None:
        project = db.query(Project).filter(Project.id == project_id).first()
        if not project:
            return None
